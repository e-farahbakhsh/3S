...
for i = 1:m2
    for j = 3:n
    Final.Basin{i,j-2} = A(k:k+WDCB_Mat(i,2)-1,j);
    end
    k = sum(WDCB_Mat(1:i,2))+1;
end
...
for i = 1:m2
    for j = 1:n-2
        for k = 1:n1+1
            Final.Score{i,j}(1,k) = k.*Final.Median{i,j}(1,k).*(Final.Number{i,j}(1,k)./WDCB_Mat(i,2)).*100;
        end
    end
end
...
for i = 1:m2
    for j = 1:n-2
        WDCB_Mat(i,j+2) = sum(Final.Score{i,j});
    end
end

for i = 1:n-2
    ss(1,i) = (2.*log(99))./(max(WDCB_Mat(:,i+2))-min(WDCB_Mat(:,i+2)));
    ii(1,i) = (max(WDCB_Mat(:,i+2))+min(WDCB_Mat(:,i+2)))./2;
end

for i = 1:m2
    for j = 1:n-2
        for k = 1:n1+1
            WDCB_Mat(i,j+n) = 1./(1+(exp((-1.*ss(1,j)).*(WDCB_Mat(i,j+2)-ii(1,j)))));
        end
    end
end